 ...
 *
 * Клиент НЕ содержит метода на каждую операцию контракта, а их восемь сотен с
 * лишним. Метод на каждую — это ещё один список, который расходится с
 * контрактом молча. Вместо этого один вызов `call(operationId, …)`,
 * типизированный сгенерированной картой `OperationTypes`: имя операции
 * проверяет компилятор, форму параметров и ответа — тоже.
 *
