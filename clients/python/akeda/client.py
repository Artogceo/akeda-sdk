...
лишним. Метод на каждую — это ещё один список, который расходится с контрактом
молча. Вместо этого один вызов ``call(operation_id, …)``, а форма операции
берётся из сгенерированной карты ``OPERATIONS``.
