...
ALLOWED: dict[tuple[str, str], str] = {
    ...
    ("clients/go/akeda/client_test.go", "ключ кабинета ak_"):
        "то же, синтетический ключ теста",
    ("examples/stock-abc/secret_test.go", "токен установки ai_"):
        "синтетический токен той же формы: тест проверяет, что секрет НЕ уходит в журнал, "
        "и без значения нужной формы проверять нечего — заменив его на «токен», "
        "мы проверяли бы фильтр на строке, которую он и так не ищет",
}
