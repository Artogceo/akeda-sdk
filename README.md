...
**Есть.** Публичный REST `/api/v1/*` под ключом кабинета: 858 внешних операций,
из них 30 стадии `public` с зафиксированной формой. Под токеном установки `ai_…`
из них достижимы 538 — чтение и запись в `core`, `marketplace`, `stock`,
`finance`, `tasks` и `calendar`. Слой ссылок на справочники. Схема манифеста
расширения и каталог точек, слотов и мест интерфейса — включая графы, которые
приложение добавляет карточкам кабинета, и функции, которые платформа исполняет
у себя в песочнице. Контракт подписанной доставки событий с векторами. Проверка
приёмника событий, не требующая живой Akeda.

**Нет, и SDK этого не изображает.** Публикации версии: завести приложение и
версию разработчик может сам, а открывает версию платформа — снаружи виден
только отчёт о готовности. Байты артефакта функции кладёт персонал платформы:
портала функций у издателя нет, манифест объявляет только отпечаток, а само
исполнение песочницы выключено на всех стендах. Токен установки выдаёт человек:
публичного обмена учётных данных на токен и самообслуживаемой ротации нет.
`akeda apps` показывает доступное по снимку и называет отсутствующее прямым
текстом, вместо того чтобы выдумывать вызов.
...
Ключ кабинета `ak_…` и токен установки `ai_…` видят контракт по-разному, и это
не выводится ни из стадии, ни из права. Что открыто установке — спрашивайте
прямо:

```bash
go run ./cmd/akeda contract reach
go run ./cmd/akeda contract op coreGetRegisterTurnovers
```
...
| Каталог | Что это |
|---|---|
| `snapshot/` | снимок релиза: OpenAPI, схема манифеста, контракт доставки, схемы слоя ссылок, каталог точек и мест, опись с контрольными суммами |
...
- [docs/extension-points.md](docs/extension-points.md) — точки, функции, слоты, места, графы и темы событий
