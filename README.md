...
**Есть.** Публичный REST `/api/v1/*` под ключом кабинета: 817 внешних операций,
из них 30 стадии `public` с зафиксированной формой. Под токеном установки `ai_…`
из них достижимы 530 — чтение и запись в `core`, `marketplace`, `stock`,
`finance`, `tasks` и `calendar`. Слой ссылок на справочники. Схема манифеста
расширения и каталог точек, слотов и мест интерфейса. Контракт подписанной
доставки событий с векторами. Проверка приёмника событий, не требующая живой
Akeda.
...
только отчёт о готовности. Токен установки выдаёт человек: публичного обмена
учётных данных на токен и самообслуживаемой ротации нет. `akeda apps` показывает
доступное по снимку и называет отсутствующее прямым текстом, вместо того чтобы
выдумывать вызов.
...
- [docs/extension-points.md](docs/extension-points.md) — точки, слоты, места и темы событий
