...
выдаёт человек — публичного обмена учётных данных на токен нет. Стенда
разработчика `sandbox.akeda.ru` **ещё не существует**. `akeda apps` говорит это
прямым текстом вместо того, чтобы выдумывать вызов.
