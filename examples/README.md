...
| `stock-abc/` | целое расширение: две страницы в слотах Akeda, свой сервер, манифест |

## Целое расширение

`stock-abc/` — не отрывок, а работающее приложение: ABC-XYZ анализ запасов с
панелью на карточке товара, страницей настройки, мостом сообщений и своим
сервером на Go. Оно только читает и показывает, чем такое расширение
отличается от вызова API: контекст запуска, псевдоним человека, тема кабинета,
два языка. Подробно — [stock-abc/README.md](stock-abc/README.md), включая
